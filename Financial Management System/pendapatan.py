...
FILE_NAME = "pendapatan.xlsx"
selected_item = None

# Simpan ke Excel

def simpan_pendapatan(tanggal, kategori, jumlah, keterangan):
...
def tambah_pendapatan():
    ...
    simpan_pendapatan(tanggal, kategori, jumlah, keterangan)
...
def update_total():
    ...
    for row in load_pendapatan():
        total += float(row[2])

    label_total.config(
        text=f"💰 Total Pendapatan : Rp {total:,.0f}"
    )
...
root = tk.Tk()
root.title("🌸 Aplikasi Pendapatan Uang 🌸")
